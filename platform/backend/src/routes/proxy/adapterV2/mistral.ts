 ...
 * Mistral Adapter
 *
 * Mistral exposes an OpenAI-compatible API, so this adapter is largely based on the OpenAI adapter.
  ...
  implements LLMStreamAdapter<MistralStreamChunk, MistralResponse>
{
  ...
  processChunk(chunk: MistralStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage first - Mistral (like OpenAI) sends usage in a final chunk with empty choices[]
...
async function convertToolResultsToToon(
  ...
  // Use OpenAI tokenizer since Mistral uses similar tokenization for most models
...
> = {
  ...
  extractApiKey(headers: MistralHeaders): string | undefined {
    // Mistral requires API keys for authentication
  ...
  createClient(
    ...
    // Mistral uses OpenAI SDK since it's OpenAI-compatible
    // Use dummy API key if none provided (Mistral typically doesn't require auth)
  ...
  extractErrorMessage(error: unknown): string {
    // Mistral uses OpenAI SDK error structure
    const MistralMessage = get(error, "error.message");
    if (typeof MistralMessage === "string") {
