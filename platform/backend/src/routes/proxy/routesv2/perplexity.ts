...
const perplexityProxyRoutesV2: FastifyPluginAsyncZod = async (fastify) => {
  ...
  // Only register HTTP proxy if Perplexity is configured (has baseUrl)
  // Routes are always registered for OpenAPI schema generation
  if (config.llm.perplexity.enabled) {
    await fastify.register(fastifyHttpProxy, {
      upstream: config.llm.perplexity.baseUrl as string,
      prefix: API_PREFIX,
      rewritePrefix: "",
    ...
    logger.info(
      "[UnifiedProxy] Perplexity base URL not configured, HTTP proxy disabled",
    );
  }
    ...
    async (request, reply) => {
      if (!config.llm.perplexity.enabled) {
        return reply.status(500).send({
          error: {
            message:
              "Perplexity provider is not configured. Set ARCHESTRA_PERPLEXITY_BASE_URL to enable.",
