...
const MistralProxyRoutesV2: FastifyPluginAsyncZod = async (fastify) => {
  ...
  // Only register HTTP proxy if Mistral is configured (has baseUrl)
  // Routes are always registered for OpenAPI schema generation
  if (config.llm.mistral.enabled) {
    await fastify.register(fastifyHttpProxy, {
      upstream: config.llm.mistral.baseUrl as string,
      prefix: API_PREFIX,
      rewritePrefix: "",
    ...
    logger.info(
      "[UnifiedProxy] Mistral base URL not configured, HTTP proxy disabled",
    );
  }
    ...
    async (request, reply) => {
      if (!config.llm.mistral.enabled) {
        return reply.status(500).send({
          error: {
            message:
              "Mistral provider is not configured. Set ARCHESTRA_MISTRAL_BASE_URL to enable.",
