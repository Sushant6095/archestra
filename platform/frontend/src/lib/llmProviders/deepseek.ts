...
class DeepseekChatCompletionInteraction implements InteractionUtils {
  private request: archestraApiTypes.DeepseekChatCompletionRequest;
  private response: archestraApiTypes.DeepseekChatCompletionResponse;
  modelName: string;

  constructor(interaction: Interaction) {
    this.request =
      interaction.request as archestraApiTypes.DeepseekChatCompletionRequest;
    this.response =
      interaction.response as archestraApiTypes.DeepseekChatCompletionResponse;
  ...
  getToolNamesRefused(): string[] {
    // DeepSeek doesn't have explicit refusal field like OpenAI
  ...
  getToolRefusedCount(): number {
    // DeepSeek doesn't have explicit refusal tracking
    // Could check finish_reason for error responses
    let count = 0;
    for (const choice of this.response.choices) {
      if (
  ...
  private mapToUiMessage(
    message:
      | archestraApiTypes.DeepseekChatCompletionRequest["messages"][number]
      | archestraApiTypes.DeepseekChatCompletionResponse["choices"][number]["message"],
  ): PartialUIMessage {
    ...
    const deepseekRoleToUIMessageRoleMap: Record<
      archestraApiTypes.DeepseekChatCompletionRequest["messages"][number]["role"],
